...
<<>>=
operons.eset <- computeExprSet(abatch.nqt, pmcorrect.method="pmonly", summary.method="medianpolish", ids = unlist(affyoperons))
@ 
...
<<>>=
operons.i <- split(seq(along=operons.ttest), operon.name)
@ 
