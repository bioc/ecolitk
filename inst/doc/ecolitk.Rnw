...
It shows an obvious a spatial pattern: genes annotated as
- Macromolecules (cellular constituent)  biosynthesis - are
mostly located on the \verb'+' strand on the first half
of the chromosome after the ORI (origin of replication), while
they appear mostly located on the \verb'-' strand on the second half.
<<fig=TRUE>>=
library(Biobase)
...
affypos <- multiget(affyids, ecoligenomeCHRLOC)
## 'unlist' as the mapping is one-to-one
bnums <- unlist(multiget(affyids, ecoligenomeBNUM)) 
strands <- unlist(multiget(bnums, ecoligenomeBNUM2STRAND))
##
multifun <- multiget(bnums, ecoligenomeBNUM2MULTIFUN)
...
and that they are transcribed\footnote{The {\it transcription} is the
  copy of DNA into RNA}} together. Complex mecanisms can regulate
...
tmp <- multiget(ecoligenome.operon$gene.name,
...
The E. coli data used in the following are freely available at
\url{http://www.affymetrix.com/support/technical/sample_data/demo_data.affx}
For convenience, as subset of these data are included in the package:
<<>>=
...
The summary statistic for the probe sets can be done simply
<<>>=
eset <- computeExprSet(abatch.nqt, pmcorrect.method="pmonly", summary.method="medianpolish", ids=affyoperons[[18]])
@ 
